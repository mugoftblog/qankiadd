...
def test_keylistener_wrong_key():
    """
        ...
        Procedure:
            - start keylistener and register 3 fields as observers
            - press shortkey of the second field
            - press random shortkey
            - press shortkey for saving all fields
            - press shortkey to stop key listening
        Explanation:
            As all shortkeys are invalid nothing should happen
        Expectation:
    ...
    model_mng = ModelManager(cfg_wrong_key)
    global is_save_all_called
    is_save_all_called = False
    ...
    print(is_save_all_called)
    assert is_save_all_called is True, "KEY_SAVE_TUPLE_STR shortkey was not handled correctly"
